...
Question capture uses Silero speech probability, keeps the 800 ms pause as an STT segment boundary, and joins every turn-scoped segment before review. A turn ends after 2,500 ms final silence or 300 ms after mute, with 15 s to first speech, a 45 s duration bound, and a 12 s processing bound. The browser receives explicit waiting, detected, processing, received, no-speech, and error states. Final audio is flushed before processing, and the lecture stays paused until STT finishes or the learner types, retries, or cancels. The worker attaches both newly subscribed and already-published microphone tracks, which removes the connect/publish race. Override only with validated `QUESTION_SEGMENT_MS`, `QUESTION_FINAL_SILENCE_MS`, `QUESTION_MUTE_DRAIN_MS`, `QUESTION_FIRST_SPEECH_MS`, `QUESTION_MAX_DURATION_MS`, and `QUESTION_PROCESSING_MS`; invalid bounds fail startup.
